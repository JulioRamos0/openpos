...
export class ThermalDriver {
  ...
  private async sendViaNtp(
    ...
    try {
      ...
      const printer = new ThermalPrinter({
        type:         this.cfg.printer.type || "epson",
        interface:    this.cfg.printer.interface,
        characterSet: resolveCharacterSet(this.cfg.printer.characterSet),
      ...
      for (let copy = 0; copy < copies; copy++) {
        printer.raw(INIT);
        await this.writeEscPos(printer, data);
  ...
  private async sendWindows(
    ...
    try {
      ...
      // Get banner path from template header
      let imageBuffer: Buffer | null = null;
      if (this.cfg.template.header.enabled) {
        const headerItems = this.cfg.template.header.items;
        const imageItem = headerItems.find(item => item.type === "image" && item.path);
        if (imageItem?.path) {
          const bannerPath = resolve(process.cwd(), "assets", "banner.png");
          imageBuffer = await pngToEscPosBitmap(bannerPath);
        }
      }
      ...
      for (let copy = 0; copy < copies; copy++) {
        ...
        // Print image first if available
        if (imageBuffer) {
          buf.raw(imageBuffer);
          buf.println("");
        }
        
        await this.writeEscPos(buf, data);
