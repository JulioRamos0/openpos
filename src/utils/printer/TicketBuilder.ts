...
export class TicketBuilder {
  ...
  private buildFooter(data: TicketData): string[] {
    ...
    if (footer.totals.enabled) {
      for (const t of footer.totals.items) {
        ...
        if (t.size === "double" || t.size === "double-width") {
          // Para double: label en una línea, valor expandido en la siguiente
          lines.push(pad(label, this.w, t.align ?? "right"));
          const expanded = val.split("").join(" ");
          lines.push(pad(expanded, this.w, "center"));
        } else {
    ...
    if (footer.qrcode.enabled) {
      ...
      lines.push(pad("[ QR ]", this.w, "center"));
      lines.push(pad(qrData, this.w, "center"));
    }
